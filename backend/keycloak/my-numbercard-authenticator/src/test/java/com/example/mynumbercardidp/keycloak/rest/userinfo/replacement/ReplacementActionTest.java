...
public class ReplacementActionTest {
    ...
    public void setUp() throws Exception {
        ...
        Map<String, AuthenticatedClientSessionModel> authenticatedClientSessions = new HashMap<String, AuthenticatedClientSessionModel>() {
            {
                put("key", authenticatedClientSessionModel);
            }
        };
        ...
        doReturn(userSessionProvider).when(keycloakSession).sessions();
        ...
        doReturn(uriInfo).when(keycloakUriInfo).getDelegate();
        doReturn(httpHeaders).when(httpRequest).getHttpHeaders();
        doReturn(userSessionModel).when(userSessionProvider).getUserSession(any(), any());
        doReturn(authenticatedClientSessions).when(userSessionModel).getAuthenticatedClientSessions();
        doReturn(authenticatedClientSessionModel).when(userSessionModel).getAuthenticatedClientSessionByClient(anyString());
    ...
    @ParameterizedTest
    @CsvSource({
        "1, 752bb712-055a-4091-b35e-45973c475dcc, 200, true",    // 正常系
        "2, 752bb712-055a-4091-b35e-45973c475dcc, 200, false",   // ユーザー情報なし
        "3, 752bb712-055a-4091-b35e-45973c475dcc, 400, true",    // プラットフォームレスポンスコード400
        "4, 752bb712-055a-4091-b35e-45973c475dcc, 500, true",    // プラットフォームレスポンスコード500
        "5, 752bb712-055a-4091-b35e-45973c475dcc, 503, true",    // プラットフォームレスポンスコード503
        "6, 752bb712-055a-4091-b35e-45973c475dcc, 401, true",    // プラットフォームレスポンスコード401
        "7, 752bb712-055a-4091-b35e-45973c475dcc, 410, true",    // プラットフォームレスポンスコード410
        "8, 752bb712-055a-4091-b35e-45973c475dcc, 403, true",    // プラットフォームレスポンスコード403(IllegalStateException)
        "9, dummy, 200, true",                                   // nonce値不一致
    })
    public void testReplace(Integer seq, String applicantData, Integer responseStatus, Boolean existsUser) throws Exception {

        try(
            MockedConstruction<TokenManager> tokenManager = mockConstruction(TokenManager.class,
                                                            (mock, ctx) -> {
                                                                doReturn(accessToken).when(mock).createClientAccessToken(any(), any(), any(), any(), any(), any());
                                                            });
            MockedConstruction<OIDCLoginProtocol> oidcLoginProtocol = mockConstruction(OIDCLoginProtocol.class,
                                                            (mock, ctx) -> {
                                                                doReturn(responseData).when(mock).authenticated(any(), any(), any());
                                                            });
            ...
            doReturn(responseStatus).when(platformAuthenticationResponse).getHttpStatusCode();
            ...
            if (seq == 8) {
                assertThrows(IllegalStateException.class, () -> {
                    replacementAction.replace(context, platform);
                });
            } else {

                if (seq == 1) {
                    expected = responseData;
                    Response result = replacementAction.replace(context, platform);
                    ...
                    if (seq == 9) {
                        ...
                        // プラットフォームレスポンスコード500
                        expected = Response.status(Response.Status.fromStatusCode(500)).build();
                    } else if (seq == 5) {
                        // プラットフォームレスポンスコード503
                        expected = Response.status(Response.Status.fromStatusCode(503)).build();
                    } else if (seq == 6) {
                        // プラットフォームレスポンスコード401
                        expected = Response.status(Response.Status.fromStatusCode(401)).build();
                    } else if (seq == 7) {
                        // プラットフォームレスポンスコード410
                        expected = Response.status(Response.Status.fromStatusCode(410)).build();
                    } else if (seq == 2) {
                        // ユーザー情報なし
                        expected = Response.status(Response.Status.NOT_FOUND).build();
                    }
