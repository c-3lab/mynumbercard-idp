...
public class CustomAttributeProviderTest {
    ...
    public void setup() throws URISyntaxException, JsonProcessingException {
        ...
        Mockito.when(session.getContext().getUri()).thenReturn(keycloakUriInfo);
        Mockito.when(session.getProvider(SingleUseObjectProvider.class)).thenReturn(singleUseObjectProvider);
        Mockito.when(session.sessions()).thenReturn(userSessionProvider);
        Mockito.when(session.sessions().getUserSession(any(), any())).thenReturn(userSession);
        Mockito.when(context.getRequestHeaders()).thenReturn(httpHeaders);
        Mockito.when(context.getRealm()).thenReturn(realm);
        Mockito.when(headers.getRequestHeaders()).thenReturn(headerMap);
        Mockito.when(realm.getClientByClientId(any())).thenReturn(client);
        Mockito.when(userSession.getAuthenticatedClientSessionByClient(any())).thenReturn(AuthenticatedClientSessionModel);
        Mockito.when(userSession.getUser()).thenReturn(user);
        Mockito.when(objectMapper.writeValueAsString(accessTokenResponseBuilder)).thenReturn(accessTokenString);
        Mockito.when(jsonNode.isObject()).thenReturn(false);
    }
    ...
    public void testSetAttributes() throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, JsonProcessingException {
        try (
            ...
            MockedConstruction<RealmManager> realmManagerConstruction = mockConstruction(RealmManager.class,
                                                    (mock, ctx) -> {
                                                        when(mock.getRealmByName(any())).thenReturn(realm);
                                                    });
            MockedConstruction<BearerTokenAuthenticator> bearerTokenAuthenticator = mockConstruction(BearerTokenAuthenticator.class,
                                                    (mock, ctx) -> {
                                                        when(mock.setRealm(any())).thenReturn(mock);
                                                        when(mock.setUriInfo(any())).thenReturn(mock);
                                                        when(mock.setTokenString(any())).thenReturn(mock);
                                                        when(mock.setConnection(any())).thenReturn(mock);
                                                        when(mock.setHeaders(any())).thenReturn(mock);
                                                        when(mock.authenticate()).thenReturn(authResult);
                                                    });
            MockedConstruction<TokenManager> tokenManagerConstruction = mockConstruction(TokenManager.class,
                                                    (mock, ctx) -> {
                                                        when(mock.responseBuilder(any(), any(), any(), any(), any(), any())).thenReturn(accessTokenResponseBuilder);
                                                        when(mock.responseBuilder(any(), any(), any(), any(), any(), any()).generateAccessToken()).thenReturn(accessTokenResponseBuilder);
                                                        when(mock.responseBuilder(any(), any(), any(), any(), any(), any()).generateAccessToken().generateIDToken()).thenReturn(accessTokenResponseBuilder);
                                                        when(mock.responseBuilder(any(), any(), any(), any(), any(), any()).generateAccessToken().generateIDToken().generateRefreshToken()).thenReturn(accessTokenResponseBuilder);
                                                        when(mock.responseBuilder(any(), any(), any(), any(), any(), any()).generateAccessToken().generateIDToken().generateRefreshToken().build()).thenReturn(accessTokenResponse);
                                                        when(mock.createClientAccessToken(any(), any(), any(), any(), any(), any())).thenReturn(token);
                                                    });
        ) {
            Mockito.when(session.getContext().getConnection()).thenReturn(connection);
            Mockito.when(session.getContext().getUri()).thenReturn(keycloakUriInfo);
            Mockito.when(authResult.getUser()).thenReturn(user);
            Mockito.when(client.getClientId()).thenReturn("sample-client");
    ...
    public void testSetAttributesWithoutRealm() {
        try (
            ...
            MockedConstruction<RealmManager> realmManagerConstruction = mockConstruction(RealmManager.class,
                                                    (mock, ctx) -> {
                                                        when(mock.getRealmByName(any())).thenReturn(null);
                                                    });
            MockedConstruction<BearerTokenAuthenticator> bearerTokenAuthenticator = mockConstruction(BearerTokenAuthenticator.class,
                                                    (mock, ctx) -> {
                                                        when(mock.setRealm(any())).thenReturn(mock);
        ...
        try (
            ...
            MockedConstruction<BearerTokenAuthenticator> bearerTokenAuthenticator = mockConstruction(BearerTokenAuthenticator.class,
                                                    (mock, ctx) -> {
                                                        ...
                                                        when(mock.setConnection(any())).thenReturn(mock);
                                                        when(mock.setHeaders(any())).thenReturn(mock);
                                                        when(mock.authenticate()).thenReturn(null);
                                                    });
        ) {
            appAuthManagerStatic.when(() -> AppAuthManager.extractAuthorizationHeaderToken(any())).thenReturn("eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJrMTVBeF8tc0pCVWNlZzRnbDZXajRnVVd3aHFjcjhJUkNsUFFCeFhFTEFnIn0..LD6Pg76hBFePAP69tzW3Sg1asP89sVZop7NnW9BlH1smHmPn-UifCq2NNmjA7KrEGSuf9rqWp_6lQ_L6osdBxBvu1S1bearjzeME8h6rWSqcyvzrdaVaGZd5Xzchl5ULZfmeNP_X1yQEIHCWvko-03X3yE5gJ3H33JHrAsf7isFo-B_qhZDv77zAr4uNJYt9wzlC7jsq39G0sKrxjGquqG1dJpoOxARVAO0pKzemr2SzzvDJfn2sMch1F-Lz8ZI60qxZ5wXRPPJMjM1WP65oEGuJLRFStDdbM_s9Avue2ErEuPztqyaDFdOhpgm8YKScrYi_mcT5LBY0evOsU-rPKw");
            assertNotNull(customAttributeProvider.setAttributes("{\"user_attributes\":{\"service_id\":\"example@example.com\",\"notes\":\"RP1\"}}"));
