...
public class ReplacementActionTest {
	...
	@Mock
	ReplacementFlowTransition flowTransition;
	@Mock
	...
	public void testAuthenticate(String applicantData, Boolean isCheckedStatusCode, Boolean existsUser) throws Exception {
		...
		try(
			...
			Field replacementActionField = replacementAction.getClass().getDeclaredField("flowTransition");
			replacementActionField.setAccessible(true);
			replacementActionField.set(replacementAction, flowTransition);

			doReturn(isCheckedStatusCode).when(flowTransition).canExecuteReplacement(any(), any());
			...
			if (applicantData.equals("dummy")) {
				...
				verify(flowTransition, times(1)).canExecuteReplacement(any(), any());
				...
				verify(flowTransition, times(1)).canExecuteReplacement(any(), any());
				verify(platformAuthenticationResponse, times(1)).getUniqueId();
				verify(platformAuthenticationResponse, times(1)).toUserModelAttributes(any());
				verify(userIdentityToModelMapper, times(1)).find(any(), any());
				userIdentityToModelMapperStatic.verify(() -> UserIdentityToModelMapper.getUserIdentityToCustomAttributeMapper(any()), times(1));
				encryptionStatic.verify(() -> Encryption.decrypt(any(), any()), times(1));
			}
