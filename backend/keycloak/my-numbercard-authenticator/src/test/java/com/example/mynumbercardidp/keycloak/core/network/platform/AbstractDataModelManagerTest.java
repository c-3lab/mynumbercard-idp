...
import com.example.mynumbercardidp.keycloak.network.platform.PlatformAuthenticationRequest;
...
public class AbstractDataModelManagerTest {

    public class ConcreteImpl extends AbstractDataModelManager {

        @Override
        protected AuthenticationRequest toUserRequest(MultivaluedMap<String, String> formData) {
            return authenticationRequest;
        }

        @Override
        protected Object toPlatformRequest(AuthenticationRequest userRequestStructure) {
            return (Object) platformAuthenticationRequest;
        }

        @Override
        protected PlatformAuthenticationResponseStructure toPlatformResponse(CloseableHttpResponse httpResponse) {
            return platformAuthenticationResponseStructure;
        }

    }

    private MultivaluedMap<String, String> userFormData = new MultivaluedHashMap<>();
    ...
    public void setup() {
        ...
        userFormData.putSingle("mode", "login");
        userFormData.putSingle("encryptedUserAuthenticationCertificate", certificateStr);
        userFormData.putSingle("sign", signStr);
        userFormData.putSingle("encryptedDigitalSignatureCertificate", "");
        userFormData.putSingle("applicantData", "752bb712-055a-4091-b35e-45973c475dcc");
        ...
        platformAuthenticationRequest = new PlatformAuthenticationRequest("sender") {
            {
                setCertificateType(CertificateType.ENCRYPTED_DIGITAL_SIGNATURE);
                setCertificate(certificateStr);
                setApplicantData("752bb712-055a-4091-b35e-45973c475dcc");
    ...
    @Test
    public void testSetPlatformRequestSender() {
        String expected = "sender";
        concreteImpl.setPlatformRequestSender("sender");
        assertEquals(expected, concreteImpl.getPlatformRequestSender());
    }

    @Test
    public void testSetUserFormData() {
        AuthenticationRequest expected = authenticationRequest;
        concreteImpl.setUserFormData(userFormData);
        assertEquals(expected, concreteImpl.getUserRequest());
    }

    @Test
    public void testSetRequestCharset() {
        Charset expected = Charset.forName("UTF-8");
        concreteImpl.setRequestCharset(Charset.forName("UTF-8"));
        assertEquals(expected, concreteImpl.getRequestCharset());
    }
    ...
    public void testGetPlatformRequest() {
        concreteImpl.setUserFormData(userFormData);
        concreteImpl.getUserRequest();
        Object expected = (Object) platformAuthenticationRequest;
    ...
    public void testGetPlatformResponse() {
        PlatformAuthenticationResponseStructure expected = platformAuthenticationResponseStructure;
        PlatformAuthenticationResponseStructure result = concreteImpl.getPlatformResponse();
        assertEquals(expected, result);
    }

    @Test
    public void testSetPlatformResponseFromHttpResponse() {
        PlatformAuthenticationResponseStructure expected = platformAuthenticationResponseStructure;
        CloseableHttpResponse httpResponse = mock(CloseableHttpResponse.class);
        concreteImpl.setPlatformResponseFromHttpResponse(httpResponse);
        assertEquals(expected, concreteImpl.getPlatformResponse());
    }
}
