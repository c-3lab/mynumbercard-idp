  ...
  public void testSendRequest(String actionMode) throws Exception {
    ...
    try(
      MockedConstruction<HttpClientBuilder> httpClientBuilder = mockConstruction(HttpClientBuilder.class,
                                                     (mock, ctx) -> {
                                                      when(mock.disableTrustManager()).thenReturn(mock);
                                                      when(mock.establishConnectionTimeout(10000L, TimeUnit.MILLISECONDS)).thenReturn(mock);
                                                      when(mock.maxConnectionIdleTime(30000L, TimeUnit.MILLISECONDS)).thenReturn(mock);
                                                      when(mock.socketTimeout(30000L, TimeUnit.MILLISECONDS)).thenReturn(mock);
                                                      when(mock.build()).thenReturn(closeableHttpClient);
                                                     });
