...
public class RegistrationActionTest {
  ...
  @Mock
  RegistrationFlowTransition flowTransition;
  @Mock
  ...
  public void setUp() throws Exception {
    ...
    when(keyManager.getActiveKey(realmModel, KeyUse.ENC, "RSA-OAEP-256")).thenReturn(keyWrapper);
    when(keycloakSession.users()).thenReturn(userpProvider);
    when(userpProvider.addUser(any(), any())).thenReturn(userModel);
    when(context.getAuthenticationSession()).thenReturn(authenticationSessionModel);
    when(context.getAuthenticationSession().getAuthNote("nonce")).thenReturn("752bb712-055a-4091-b35e-45973c475dcc");
    when(context.getRealm()).thenReturn(realmModel);
    when(platformAuthenticationResponse.toUserModelAttributes(userModel)).thenReturn(userModel);
    ...
    doNothing().when(userModel).setEnabled(anyBoolean());
  }
  ...
  public void testAuthenticate(String applicantData, Boolean isCheckedStatusCode, Boolean existsUser) throws Exception {
    ...
    try(
      ...
      Field registrationActionField = registrationAction.getClass().getDeclaredField("flowTransition");
      registrationActionField.setAccessible(true);
      registrationActionField.set(registrationAction, flowTransition);

      doReturn(isCheckedStatusCode).when(flowTransition).canExecuteRegistration(any(), any());

      UserModel returnedUserModel = userModel;
      if (!existsUser) {
        returnedUserModel = null;
      }

      doReturn(returnedUserModel).when(userIdentityToModelMapper).find(any(), any());
      ...
      // 実行結果
      registrationAction.register(context, platform);
      ...
      if (applicantData.equals("dummy")) {
        ...
        verify(context, times(2)).getAuthenticationSession();
        ...
        verify(flowTransition, times(1)).canExecuteRegistration(any(), any());
        encryptionStatic.verify(() -> Encryption.decrypt(any(), any()), times(1));
      } else if (!existsUser) {
        verify(context, times(2)).getSession();
        verify(context, times(2)).getAuthenticationSession();
        verify(context, times(2)).getRealm();
        verify(context, times(1)).setUser(any());
        verify(context, times(1)).success();
        verify(userpProvider, times(1)).addUser(any(), any());
        verify(userModel, times(1)).setEnabled(anyBoolean());
        verify(platform, times(3)).getUserRequest();
        verify(platform, times(1)).getPlatformResponse();
        verify(platform, times(1)).sendRequest();
        verify(flowTransition, times(1)).canExecuteRegistration(any(), any());
        verify(platformAuthenticationResponse, times(1)).getUniqueId();
        verify(platformAuthenticationResponse, times(1)).toUserModelAttributes(userModel);
